...
impl UmodeBuffer {
    ...
    pub fn map(sv48: &FirstStagePageTable<Sv48>, hyp_mem: &mut HypPageAlloc) -> UmodeBuffer {
        ...
        {
            // Safety: These pages will be written by the hypervisor using the physical hypervisor
            // mappings and read by U-mode through this mapping in read only. Safe because these are
            // per-CPU mappings, and when the hypervior will be writing to these pages via the
