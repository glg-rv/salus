...
impl<'a, T: GuestStagePagingMode> FinalizedVm<'a, T> {
    ...
    fn guest_test_memcopy(&self, to: u64, from: u64, len: u64) -> EcallResult<u64> {
        ...
        let (to_vaddr, _to_mapping) = self.map_guest_range_in_umode_slot(1, to, len, true)?;
        // The two ranges do not overlap, and we mapped them for at least `len` bytes, `to_vaddr`
        // writable and `from_vaddr` readable.
