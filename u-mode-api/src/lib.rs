...
impl UmodeRequest {
    ...
    /// # Safety
    ///
    /// The ranges (in_addr + len) and (out_addr + len) must be valid and not overlap.
    /// The range (in_addr + len) must not be mutated.
    pub unsafe fn memcopy(out_addr: u64, in_addr: u64, len: u64) -> UmodeRequest {
        UmodeRequest {
            op: UmodeOp::MemCopy,
    ...
    pub fn input(&self) -> Option<&[u8]> {
        if let Some(addr) = self.in_addr {
            ...
            // memory will not be mutated while mapped.
            unsafe {
    ...
    pub fn output(&self) -> Option<&mut [u8]> {
        if let Some(addr) = self.out_addr {
            ...
            // for reading and writing, and that the memory will not be mutated by the hypervisor
            // while mapped.
            unsafe {
